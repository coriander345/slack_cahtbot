...
SCHEDULED_MESSAGES = [
    {'text': 'First message', 'post_at': (
        datetime.now() + timedelta(seconds=20)).timestamp(), 'channel': 'C04UF32E97Y'},
    {'text': 'Second Message!', 'post_at': (
        datetime.now() + timedelta(seconds=30)).timestamp(), 'channel': 'C04UF32E97Y'}
]

client.chat_postMessage(channel='C04UF32E97Y', text="dd")
...
if __name__ == "__main__":
    ...
    ids = list_scheduled_messages('C04UF32E97Y')
    delete_scheduled_messages(ids, 'C04UF32E97Y')
    app.run(debug=True)